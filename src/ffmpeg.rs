...
/// Returns the duration (in seconds) of the specified audio stream, or None if not available.
pub fn get_audio_stream_duration(
    input_file: &str,
    stream_index: usize,
) -> Result<Option<f64>, FFmpegError> {
    ...
    if !output.status.success() {
        return Err(FFmpegError::CommandFailed(
            "get_audio_stream_duration".to_string(),
            String::from_utf8_lossy(&output.stderr).to_string(),
        ));
    }
